...
本 Skill 是本仓库 Legion orgmode 工作流的 AI 执行指南。
...
1. 默认任务 ID 已配置且可解析：
   ...
   - 若多个文件复用了同一个 `:ID:`，`<Leader>opI` 会优先命中当前 buffer；长期仍建议保持 ID 全局唯一
2. 可选日志文件：
...
| 操作 | 快捷键 / 命令 | 预期行为 |
| --- | --- | --- |
| 当前条目 clock in | `<Leader>oxi`（或 agenda `I`） | 应用 Legion 的 TODO/NEXT 自动切换规则 |
...
| 操作 | 快捷键 / 命令 | 预期行为 |
| --- | --- | --- |
| 打开 capture | `<Leader>X` | 使用 Legion 模板，并暂停当前 clock（handoff） |
...
3. 审阅影响后，仅在用户明确确认后执行 `:OrgLegionCleanupDerivedTags!`。
4. 保存受影响 buffer。
...
- 若观察到行为与本 Skill 不一致：引用具体差异，并以 `skills/legion-workflow/references/orgmode-legion-workflow.md` 为准。
