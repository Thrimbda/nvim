## 变更内容
- 在 `lua/tests/smoke/orgmode_smoke.lua` 中完成 Norang parity smoke 更新，并落地 `PARITY_FAIL {json}` 协议。
- 新增 `clock_out_in_punch_mode_returns_to_parent` 与 `norang_e2e_integrated_flow`，覆盖父任务/默认任务回退与完整集成流程。
- 收紧 capture pre-refile 校验到源 subtree 范围，并更新 parity 边界文档。

## 变更原因
- 需要一个可靠的 fail-fast 对齐门禁，验证端到端 Norang 行为，而不仅是离散断言。
...
- 报告：`.legion/tasks/orgmode-norang-e2e-parity-suite/docs/test-report.md`
- 评审：code PASS（`.legion/tasks/orgmode-norang-e2e-parity-suite/docs/review-code.md`），security PASS（`.legion/tasks/orgmode-norang-e2e-parity-suite/docs/review-security.md`）
...
- Walkthrough：`.legion/tasks/orgmode-norang-e2e-parity-suite/docs/report-walkthrough.md`
