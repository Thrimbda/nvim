# Design Lite：Norang Workflow AI Skill

## 目标

将现有 Norang 工作流文档压缩为 AI 可执行的 skill 指南，降低重复解释与执行偏差。

## 输入与事实来源

1. 主真源：`docs/orgmode-norang-workflow.md`
2. 交叉参考：`lua/org_punch.lua`、`lua/org_capture_norang.lua`
3. 任务约束：仅在 scope 内新增 skill 资产，不修改运行时代码

## 产出设计

- 新增：`skills/norang-workflow/SKILL.md`
- 内容结构：
  - 适用场景与目标
...
- 若需回滚，删除 `skills/norang-workflow/` 与本任务 `.legion` 产物即可。
