# 交付 Walkthrough：Legion Workflow AI Skill

## 目标与范围

- 目标：将既有 Legion 工作流文档产品化为 AI 可执行 skill，降低重复解释和执行口径偏差。
- 绑定 scope：`skills/legion-workflow/**` 与本任务文档产物，未触及运行时代码路径（如 `lua/**`）。
- 本次交付类型：Low 风险、文档型变更。

## 设计摘要

- 设计依据：`./.legion/tasks/legion-workflow-ai-skill/docs/design-lite.md`。
- 事实来源优先级：以 `skills/legion-workflow/references/orgmode-legion-workflow.md` 为主真源，skill 作为可执行封装；若冲突以真源为准。
- 关键设计点：
...
- 文件：`skills/legion-workflow/SKILL.md`
- 改动内容：
...
- 文件：`skills/legion-workflow/references/quick-checklist.md`
- 改动内容：
...
- 文件：`./.legion/tasks/legion-workflow-ai-skill/docs/test-report.md`
  - 记录文档型验证结果（PASS）与检查覆盖项。
- 文件：`./.legion/tasks/legion-workflow-ai-skill/docs/review-code.md`
...
- 命令执行结果为 `PASS`。
- `skills/legion-workflow/SKILL.md` 与 `skills/legion-workflow/references/quick-checklist.md` 存在。
- `SKILL.md` 必含关键章节：
  - `## Source of truth`
...
- 测试报告：`./.legion/tasks/legion-workflow-ai-skill/docs/test-report.md`
- 评审报告：`./.legion/tasks/legion-workflow-ai-skill/docs/review-code.md`
