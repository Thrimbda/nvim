# 实现走查：orgmode-legion-e2e-parity-suite

## 1) 目标与范围
- 目标：完成一个可用于 CI 的 Legion 对齐 E2E smoke 门禁，具备清晰失败语义与稳定回退行为。
- 范围内文件：
...
## 2) 设计摘要（与 RFC 对齐）
- 设计来源：`.legion/tasks/orgmode-legion-e2e-parity-suite/docs/rfc.md`。
...
- 新增 case：`legion_e2e_integrated_flow`。
...
- 最终结果：PASS `16/16`（见 `.legion/tasks/orgmode-legion-e2e-parity-suite/docs/test-report.md`）。
- 评审门禁：code PASS（`.legion/tasks/orgmode-legion-e2e-parity-suite/docs/review-code.md`），security PASS（`.legion/tasks/orgmode-legion-e2e-parity-suite/docs/review-security.md`）。
...
- Test report：`.legion/tasks/orgmode-legion-e2e-parity-suite/docs/test-report.md`
