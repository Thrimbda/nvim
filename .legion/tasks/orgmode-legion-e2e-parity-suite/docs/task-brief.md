# 任务简报：Legion E2E 对齐套件

## 问题定义

当前仓库已有多组 org/legion smoke 用例，但尚缺一个“按 Legion 原文流程串联”的端到端校验入口。目标是在单个 E2E 场景中覆盖关键行为，并以可执行断言验证实现是否忠于：

1. Legion 工作流原文（skills/legion-workflow/references/orgmode-legion-workflow.md）
2. 上游 orgmode/org-mode.nvim 的能力边界
3. 本仓库当前实现（org_punch/org_capture_legion/org_legion + orgmode 配置）
...
- 补充文档，明确该 E2E 用例覆盖的 Legion 关键行为与边界。
...
- 涉及多模块协同（orgmode 配置 + org_punch + org_capture_legion + org_legion + smoke harness）。
- 验收口径来自外部规范（Legion 原文）且需与上游 orgmode 语义兼容，设计错误会导致测试“错测”。
...
- Legion 原文与 orgmode.nvim 行为存在历史差异时，测试需优先断言“本仓库宣称支持的行为”，并在 RFC 中明确不覆盖项。
- capture/clock 用例存在时间粒度抖动风险（分钟边界），需使用稳定断言（例如避免依赖精确秒数）。

## 外部参考

- Legion 原文：skills/legion-workflow/references/orgmode-legion-workflow.md
