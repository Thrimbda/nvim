...
## 执行摘要
- **问题**：当前 smoke 测试验证的是离散的 Legion 行为，但没有一个端到端、Legion 风格的工作流对齐门禁，无法在 CI 中快速失败。
- **决策**：新增一个集成的 headless Neovim 流程测试与辅助断言，并维护一个显式行为矩阵，绑定三个基线：仓库实现、orgmode.nvim 边界、Legion 参考工作流。
- **为何现在做**：该任务属于 Epic/Medium-risk，且依赖外部语义；没有成文证据模型就容易“测错对象”。
- **影响**：提升对 punch、clock、capture handoff、TODO tag trigger 与 derived-tag 生命周期与声明的 Legion 行为保持一致的信心。
- **风险**：时间边界抖动、过度断言 UI 行为、以及 Legion 文章与 orgmode.nvim 能力不一致。
...
## 1. 背景 / 动机
- 仓库在 `lua/tests/smoke/orgmode_smoke.lua` 已有聚焦型 smoke case，但缺少一个从 punch-in 到 capture handoff 再返回、能镜像 Legion 日常流程的单场景测试。
- task brief 要求一个可执行的对齐门禁，并锚定在：
  1) Legion 原文（`skills/legion-workflow/references/orgmode-legion-workflow.md`），
  2) orgmode.nvim 行为边界（`.tests/deps/orgmode/doc/orgmode.txt`），
  3) 本仓库实际模块（`lua/org_punch.lua`、`lua/org_capture_legion.lua`、`lua/org_legion/init.lua`、`lua/org_legion/todo_triggers.lua`）。
- 缺少此门禁会导致语义回归仍可能通过离散测试。

## 2. 目标
- 新增一个集成 E2E smoke case，在 `nvim --headless` 下验证 Legion 关键序列。
...
- **范围**：修改限制在 `lua/tests/smoke/orgmode_smoke.lua`、`tests/smoke/run.sh`、`skills/legion-workflow/references/orgmode-legion-workflow.md` 与任务文档。
...
- **Baseline C**：Legion 工作流意图（外部参考行为）。
...
### 6.1 基线
- **A（Repo）**：`lua/org_punch.lua`、`lua/org_capture_legion.lua`、`lua/org_legion/init.lua`、`lua/org_legion/todo_triggers.lua`、`lua/tests/smoke/orgmode_smoke.lua`、`tests/smoke/run.sh`、`skills/legion-workflow/references/orgmode-legion-workflow.md`。
- **B（orgmode.nvim）**：`.tests/deps/orgmode/doc/orgmode.txt` 中 TODO 状态、clocking、capture 与映射相关章节。
- **C（Legion）**：`skills/legion-workflow/references/orgmode-legion-workflow.md` 中工作流期望（持续 clocking、NEXT 语义、capture/punch 约定）。

### 6.2 证据层级
1. **可执行证据优先**：smoke case 中的 headless 断言。
2. **实现证据其次**：仓库模块内函数级行为。
3. **文档证据再次**：orgmode.nvim 文档与 Legion 文档用于意图/边界。
...
### 7.1 高层架构
- 保持现有 smoke harness 形态（`run(case_name)` 分发），新增一个集成 case，例如 `legion_e2e_integrated_flow`。
- 保留辅助 case 作为原子化诊断（每个 case 只覆盖一个行为）。
- 在 runner（`tests/smoke/run.sh`）中把集成 case 放在 Legion 相关 case 附近，保持确定性顺序。
...
### 7.5 失败诊断格式
- 将断言失败统一为单行 JSON 载荷：
  - `PARITY_FAIL {"v":1,"id":"NP-006B","phase":"clock","case":"legion_e2e_integrated_flow","expected":"clock title=Organization","actual":"clock title=nil"}`
...
| NP-002 | 使用有效默认 ID 执行 punch in，会在默认任务启动活动时钟并开启 keep-running。 | MUST | A-extension | `punch_in_clocks_default_task` | `lua/org_punch.lua`, `skills/legion-workflow/references/orgmode-legion-workflow.md` | 用含 `:ID:` 的临时 org，调用 `punch_in`，断言存在活动 CLOCK 行且 flag=true。 |
| NP-003 | 对 TODO 叶子任务执行 clock in，状态从 TODO -> NEXT。 | MUST | A-extension | `clock_in_todo_task_switches_to_next` | `lua/org_punch.lua`, `skills/legion-workflow/references/orgmode-legion-workflow.md` | 光标放在 TODO 叶子任务，调用 `clock_in_current_task`，断言标题前缀为 `NEXT`。 |
| NP-004 | 对 NEXT 项目执行 clock in，状态从 NEXT -> TODO。 | MUST | A-extension | `clock_in_next_project_switches_to_todo` | `lua/org_punch.lua`, `skills/legion-workflow/references/orgmode-legion-workflow.md` | 光标放在含 TODO 子任务的 NEXT 节点，clock in 后断言项目标题为 `TODO`。 |
| NP-005 | Clock out 会移除 `0:00` CLOCK 记录。 | MUST | A-extension | `clock_out_removes_zero_duration_clock` | `lua/org_punch.lua`, `skills/legion-workflow/references/orgmode-legion-workflow.md` | 立即 clock in/out，扫描文件或 buffer，断言不存在 `=> 0:00`。 |
...
| NP-006B | 在 punch 模式且不存在 TODO 父任务时，从当前任务 clock out 应恢复到默认任务。 | MUST | A-extension | `clock_out_in_punch_mode_returns_to_default` | `lua/org_punch.lua`, Legion workflow intent URL | 构造无 TODO 父任务的叶子 fixture，`punch_in` -> `clock_in` -> `clock_out`，断言活动时钟标题 == 默认任务。 |
| NP-007 | Capture handoff 在 capture 期间暂停活动时钟，并在结束后恢复之前时钟。 | MUST | A-extension | `capture_clock_handoff_resumes_previous` | `lua/org_capture_legion.lua`, `skills/legion-workflow/references/orgmode-legion-workflow.md` | 启动活动时钟，调用 `begin_capture_clock_handoff` 后再 `finish_capture_clock_handoff`，断言先为 nil 后恢复原标题。 |
| NP-008 | 当耗时 >=1 分钟时，capture pre-refile 会注入 LOGBOOK + 非零 CLOCK 行。 | MUST | A-extension | `capture_pre_refile_injects_clock_line` | `lua/org_capture_legion.lua`, `.tests/deps/orgmode/doc/orgmode.txt` (capture hooks) | 强制 started_at 早于当前 60s，调用 `on_pre_refile`，断言存在 LOGBOOK 且 `CLOCK ... => x:yy` 非 `0:00`。 |
| NP-009 | TODO trigger：WAITING 添加 WAITING；HOLD 添加 WAITING+HOLD；CANCELLED 添加 CANCELLED 并清除 WAITING/HOLD。 | MUST | A-extension | `todo_state_tag_triggers_legion` | `lua/org_legion/todo_triggers.lua`, `skills/legion-workflow/references/orgmode-legion-workflow.md` | 驱动 todo 状态迁移并逐状态校验 tag 集合增删。 |
| NP-010 | 迁移到 TODO/NEXT/DONE 时清除 WAITING/HOLD/CANCELLED 标签。 | MUST | A-extension | `todo_state_tag_triggers_legion` | `lua/org_legion/todo_triggers.lua`, `skills/legion-workflow/references/orgmode-legion-workflow.md` | 应用状态迁移，断言需移除标签不在本节点 tag 列表中。 |
| NP-011 | Derived tag refresh 会把无 next action 的项目标记为 `PROJECT:STUCK`（仓库 approx-mode 约定）。 | SHOULD | A-extension | `legion_refresh_marks_stuck_project` | `lua/org_legion/init.lua`, `lua/org_legion/refresh.lua`, `skills/legion-workflow/references/orgmode-legion-workflow.md` | 对构造项目运行 `refresh_file`，断言该行包含派生标签。 |
| NP-012 | Derived tag cleanup apply 仅移除派生标签（`PROJECT/STUCK/ARCHIVE_CANDIDATE`）。 | MUST | A-extension | `legion_cleanup_apply_removes_derived_tags` | `lua/org_legion/init.lua`, `skills/legion-workflow/references/orgmode-legion-workflow.md` | 执行 cleanup apply，断言派生标签被移除，用户自定义标签若存在则保留。 |
...
| NP-015 | 包装层使用的 orgmode.nvim clock 接口遵循文档映射/语义（`I/O`、`<Leader>oxi/oxo`、clock report）。 | SHOULD | B-public | `legion_e2e_integrated_flow` (new) | `.tests/deps/orgmode/doc/orgmode.txt` (Clocking sections), `lua/org_punch.lua` | 集成流程中校验活动时钟生命周期符合 orgmode clock API 结果（活动标题出现/消失迁移）。 |
...
- 未采纳原因：task acceptance 明确要求至少一个端到端 Legion 流程。
...
- 步骤：
  ...
  3. 更新 `skills/legion-workflow/references/orgmode-legion-workflow.md` 的对齐边界与测试映射。
...
- 回滚触发：
  ...
  - CI 时长显著回归或 flake 激增。
- 可执行回滚步骤：
  1. 编辑 `tests/smoke/run.sh`，从 `CASES` 中移除 `legion_e2e_integrated_flow`（或等效集成 case）。
  2. 回退 `lua/tests/smoke/orgmode_smoke.lua` 中的集成 case 函数（保留辅助 cases）。
  3. 更新 `skills/legion-workflow/references/orgmode-legion-workflow.md`，说明当前临时回滚状态。
  4. 执行 `bash tests/smoke/run.sh`，确认辅助 cases 仍通过。
  5. 确认日志不再包含集成 case 启动行（`==> smoke: legion_e2e_integrated_flow`）。
- 回滚后一致性：
...
- 本地调试可选命令：
  - `nvim --headless -u NONE "+set rtp+=$ORGMODE_RTP" "+set rtp+=$PWD" "+lua require('tests.smoke.orgmode_smoke').run('legion_e2e_integrated_flow')" +"qa!"`
- 通过标准：
  - 所有 MUST ID 通过。
...
- **里程碑 1：矩阵 + 诊断契约**
  ...
  - 回滚影响：无，需要时仅文档回退。
- **里程碑 2：集成流程 Case**
  - 范围：在 `lua/tests/smoke/orgmode_smoke.lua` 实现 `legion_e2e_integrated_flow` 并注册到 `CASES`。
  - 验收：case 在 headless 下通过；覆盖 punch->clock->capture->tags->refresh 序列。
  - 回滚影响：移除单个 case 条目与函数。
- **里程碑 3：Runner + 文档集成**
  - 范围：在 `tests/smoke/run.sh` 添加 case；更新 `skills/legion-workflow/references/orgmode-legion-workflow.md` 的对齐边界与矩阵引用。
...
- `.legion/tasks/orgmode-legion-e2e-parity-suite/docs/rfc.md`
  - 作为矩阵与发布策略的单一事实源。
...
- 仓库工作流文档：`skills/legion-workflow/references/orgmode-legion-workflow.md`
- Smoke harness：`lua/tests/smoke/orgmode_smoke.lua`、`tests/smoke/run.sh`
- 仓库模块：`lua/org_punch.lua`、`lua/org_capture_legion.lua`、`lua/org_legion/init.lua`、`lua/org_legion/todo_triggers.lua`
- Orgmode 文档：`.tests/deps/orgmode/doc/orgmode.txt`
- Legion 参考：`skills/legion-workflow/references/orgmode-legion-workflow.md`
- Org manual 参考：
