# nvim-orgmode-legion-implementation - 上下文
...
- 已创建任务产物目录：.legion/tasks/nvim-orgmode-legion-implementation/docs 与 reports。
- 完成 org_legion 五个模块实现（init/parser/rules/refresh/cleanup），含命令注册、刷新协议、冲突检测、cleanup dry-run/apply。
- 完成 orgmode 集成：接入 org_legion.setup，并扩展 b 视图新增 PROJECT+STUCK / PROJECT-STUCK / ARCHIVE_CANDIDATE。
- 完成 org_punch 轻量协同校验提示：TODO 语义不一致仅告警，不阻断主流程。
- 完成文档更新：补充 OrgLegion 命令、memory_only 语义、cleanup 回滚与排错说明。
- 执行最小校验：对 lua/org_legion/*.lua、lua/plugins/orgmode.lua、lua/org_punch.lua 逐个运行 nvim --headless -u NONE +"luafile ..."，7/7 成功加载。
- 输出测试报告：.legion/tasks/nvim-orgmode-legion-implementation/docs/test-report.md（结论 PASS，附 RFC T1-T16 手工验证步骤）。
...
- 已实现 refresh_unloaded_files 开关并默认开启，OrgLegionRefresh 现在可处理未加载 agenda 文件。
- 已更新文档说明新语义与开关行为。
- 已完成语法回归校验（headless luafile 通过）。
- 已依据 legion 文档 Custom Agenda Setup 补齐 Standalone Tasks 区块，解决 TODO 叶子任务在 block agenda 不可见问题。
...
- 已修复 stuck projects 判定偏差：项目子任务现在按 Legion 口径使用“任意 TODO 关键词（含 done 状态）”识别项目。
...
- 已将默认 agenda 模式从 `~/OneDrive/cone/**/*` 调整为 `~/OneDrive/cone/**/*.org`（orgmode 与 org_legion 一致）。
- 已针对用户样例文件 `/Users/c1/OneDrive/cone/course.org` 执行刷新验证，结果 `ok=true changed=true`，并写入 `:PROJECT:STUCK:` 标签。
- 已完成全量语法回归校验（org_legion + orgmode 配置）。
...
- 已完成语法校验（luafile lua/plugins/orgmode.lua 通过）。
- 完成 org-mode 相关代码整体审查（orgmode 配置、org_punch、org_legion refresh/cleanup/rules、文档）。
- 已修复 org_punch agenda 文件发现：改为仅 HOME 展开 + 通配兼容，不再因 expand+glob 组合导致匹配为空。
- 已修复 org_legion cleanup agenda 文件发现：与 refresh 逻辑统一（HOME 展开 + `/**/*` 兼容 + 目录回退）。
...
- 已对照 Legion 文档补齐 clock-in 状态切换：TODO task -> NEXT，NEXT project -> TODO。
...
- 已新增冒烟测试文件 `lua/tests/smoke/orgmode_smoke.lua`，覆盖 7 个关键场景（punch in/out、TODO/NEXT 自动切换、0:00 CLOCK 清理、legion refresh/cleanup）。
...
- 模板目标统一落地 inbox（`refile.org`）与 journal datetree（`journal.org`），与现有 Legion 流程兼容。
- 已更新工作流文档，补充 capture 类型与快捷入口说明。
- 已完成配置语法校验（luafile lua/plugins/orgmode.lua 通过）。
- 已将 Journal capture 目标从 `journal.org` 调整为 `diary.org` datetree，与 Legion 原文对齐。
- 新增 `org_capture_legion` 模块并接入：在选择 capture 模板时执行 clock handoff（暂停当前 clock），在完成/取消 capture 后恢复之前 clock。
- 已将 `<Leader>X` 切换为 Legion capture 包装入口，并保持模板快捷键使用不变。
...
- 保留 Legion clock handoff 语义：打开 capture 时暂停当前 clock，完成/取消后恢复之前 clock。
...
- 扩展后 smoke 套件全通过。
- 已补齐无自动写盘约束到 capture resume：`org_capture_legion.clock_in_snapshot` 不再使用 `OrgFile:update`，改为隐藏 buffer 内存 clock-in。
- org_punch 与 org_capture_legion 现在均采用隐藏 buffer 操作，默认不触发 `silent! w`。
- 相关 smoke 套件再次全量通过。
...
- 按 Scope 完成 Legion TODO 状态触发标签实现：新增 org_legion.todo_triggers，覆盖 WAITING/HOLD/CANCELLED 与 TODO/NEXT/DONE 的标签增删规则。
- 按 Scope 完成 Capture 模板补齐：在 orgmode 模板中新增 w（org-protocol）与 h（habit）。
- 按 Scope 更新工作流文档：同步 capture 模板清单与 TODO 状态触发标签规则。
- 修复 walkthrough 报告中的旧 RFC 路径，已改为当前 task 的 .legion/tasks/nvim-orgmode-legion-implementation/docs/rfc.md。
- 新增 smoke 用例 todo_state_tag_triggers_legion，并更新 tests/smoke/run.sh 执行列表。
- 已执行 tests/smoke/run.sh 全量冒烟，14/14 case PASS。
- 2026-03-01 执行 org/legion 冒烟验证：`bash tests/smoke/run.sh`，14/14 case PASS（含 `todo_state_tag_triggers_legion`、`legion_refresh_marks_stuck_project`、`legion_cleanup_apply_removes_derived_tags`）
- 阶段 A 已完成：按 Legion 口径补齐 TODO 状态触发、capture 模板 w/h，并更新使用文档与 smoke 用例
...
| 在 block agenda 中新增 Standalone Tasks 块，匹配 `+TODO="TODO"-PROJECT-REFILE`。 | legion 标准流程包含 Standalone Tasks 视图；当前用户 TODO 任务因缺少该块而不可见。 | 仅依赖 Next/Projects 视图；无法覆盖 TODO 叶子任务。 | 2026-02-12 |
| 在保留 block agenda 的前提下增加独立 Stuck Projects 入口。 | 符合 Legion 实际使用习惯，便于快速聚焦卡住项目而不必每次打开完整 block agenda。 | 仅保留 b 视图中的 Stuck Projects 区块；发现与操作效率较低。 | 2026-02-12 |
| 将项目判定从“有活动子任务”改为“有任意 TODO 关键字子任务”，并将 WAITING+NEXT 视为不可解除 stuck。 | 与 Legion 原始定义保持一致，修复用户样例 course.org 被漏判问题。 | 继续仅统计 active 子任务；会把仅含 DONE 子任务的项目误判为非项目。 | 2026-02-12 |
...
| 不将 CLOCK 记录直接渲染为时间线 time block，而是遵循 orgmode 语义：时间线展示时间戳，clock 通过 report 展示。 | 与 Legion/Org 行为一致，避免将日志数据误作计划数据，降低副作用。 | 将 clock-in 自动写入 SCHEDULED 时间戳以强制占位；会污染计划字段且偏离原流程。 | 2026-02-12 |
...
| 不改动 orgmode 上游 capture 内核，采用轻量包装模块对 open_template 与 capture hooks 做时钟接力。 | 最小侵入地补齐 Legion 习惯（capture 时钟切换与恢复），同时保持升级兼容性。 | 直接 patch orgmode capture 代码；维护成本高且容易与上游冲突。 | 2026-02-12 |
...
| capture clock 记录采用分钟边界差值而非秒级 floor 时长；并在 0 分钟时跳过记录。 | 与用户感知和 Legion 流程一致，避免“跨分钟却记 0:00”与无效 0 分钟污染。 | 继续使用秒级 floor；会重复触发 `0:00` 争议。 | 2026-02-12 |
...
| 本轮改用“三基线对齐审计协议”（A:实现，B:task，C:Legion）作为一致性判定口径，并以 `docs/rfc.md` 为唯一设计真源。 | 用户需求是“检查现有实现与 task/Legion 是否一致”，需要统一判级标准与证据链，避免口头结论。 | 直接给结论不建协议；可读性与可追溯性不足，后续 /legion-impl 难落地。 | 2026-03-01 |
...
1. 若用户要求合并，直接使用 `.legion/tasks/nvim-orgmode-legion-implementation/docs/pr-body.md` 作为 PR 描述。
