# RFC: nvim-orgmode Legion 插件协议规范（org_legion）

- 状态：Draft（修订版，目标 `PASS-WITH-CHANGES`）
- 最近更新：2026-02-07
- Scope：`lua/plugins/orgmode.lua`, `lua/org_punch.lua`, `lua/org_legion/**`, `skills/legion-workflow/references/orgmode-legion-workflow.md`
...
本文定义 `org_legion` 的可实现、可测试、可评审协议：
1) 派生标签 `PROJECT`/`STUCK`/`ARCHIVE_CANDIDATE`；
2) 全量与单文件刷新协议（含并发、重入、冲突与写回语义）；
3) agenda block 扩展；
4) 与 `org_punch` 的兼容契约；
5) 可回滚命令 `OrgLegionCleanupDerivedTags`（dry-run + apply）。

V1 以 `approx` 为唯一 MUST 算法档位；`precise` 降级为 MAY（实验特性）。

## 动机（Motivation）

- 当前已有 Legion-lite：`b/n/r` agenda 与 `org_punch` keep-running。
...
`require("org_legion").setup(opts)` MUST 完成：
- 注册命令 `:OrgLegionRefresh`（全量刷新）；
- 注册命令 `:OrgLegionReload`（错误态恢复）；
- 注册命令 `:OrgLegionCleanupDerivedTags[!]`（`!` 表示 apply，默认 dry-run）；
...
- `S1 -> S3`：`:OrgLegionRefresh`。
- `S2 -> S1`：单文件成功。
- `S3 -> S1`：全量全部成功。
- `S3 -> S4`：全量部分失败。
- `S4 -> S3`：用户重试 `:OrgLegionRefresh`。
- `any -> S5`：`E_CFG_INVALID` 或初始化关键失败。
- `S5 -> S1`：用户修复配置后执行 `:OrgLegionReload` 成功（幂等）。
- `S5 -> S5`：继续执行刷新命令，返回 `E_CFG_INVALID` 且不执行刷新。

错误态最小命令集：
- 允许：`:OrgLegionReload`、`:messages`。
...
- `org_legion` MUST 不调用 `org_punch` 私有实现。
- 协作边界为“共享 TODO 语义 + agenda 可见性”。
- `org_punch` keep-running 主流程 MUST 不因启用 `org_legion` 而改变。
...
- `E_CFG_INVALID`：MUST 修复配置后执行 `:OrgLegionReload`。
...
迁移：
1. 阶段 1：启用 `org_legion`，默认 `mode=approx`。
2. 阶段 2：启用 agenda 新 blocks。
3. 阶段 3：补充文档与验收脚本。

灰度：
- 先开 `on_buf_write`，再推广 `:OrgLegionRefresh` 到全库。

回滚：
1. 关闭 `org_legion.setup` 与新增 blocks。
2. 执行 `:OrgLegionCleanupDerivedTags`（dry-run 确认后 apply）。
3. 验证仅派生标签被清理，用户标签保留。
...
- R1：系统 MUST 提供 `:OrgLegionRefresh`。
- R2：系统 MUST 提供 `:OrgLegionReload`。
- R3：系统 MUST 提供 `:OrgLegionCleanupDerivedTags`（默认 dry-run，`!` 为 apply）。
...
- R20：修复配置后执行 `:OrgLegionReload` MUST 可从 `S5` 回到 `S1`。
- R21：文档 MUST 覆盖命令、错误、回滚、兼容验证步骤。

### 测试映射

- T1 -> R1：执行 `:OrgLegionRefresh`，验证汇总输出。
- T2 -> R2/R20：构造非法配置进入 `S5`，修复后 `:OrgLegionReload` 恢复到 `S1`。
...
- T11 -> R16：启用/禁用 `org_legion` 前后对比 `b/n/r` 既有块与行为一致。
- T12 -> R17：启用 `org_legion` 后执行 punch in/out keep-running，行为与旧版一致。
...
1. 在 `orgmode` 启动路径接入 `org_legion.setup`。
...
- `require("org_legion").setup(opts)`
- `require("org_legion").refresh_all()`
- `require("org_legion").refresh_file(path_or_bufnr)`
- `require("org_legion").reload()`
- `require("org_legion").cleanup_derived_tags({ apply = false })`

### 文件变更明细（设计范围）

- `lua/org_legion/init.lua`：配置、命令、autocmd、状态管理。
- `lua/org_legion/parser.lua`：headline/TODO/tag 解析。
- `lua/org_legion/rules.lua`：派生标签判定。
- `lua/org_legion/refresh.lua`：刷新事务、锁、冲突检测、汇总。
- `lua/org_legion/cleanup.lua`：派生标签清理（dry-run/apply）。
- `lua/plugins/orgmode.lua`：接入 setup 与 agenda blocks。
- `lua/org_punch.lua`：仅协同校验，不改主流程。
- `skills/legion-workflow/references/orgmode-legion-workflow.md`：命令、排错、回滚与验收说明。
