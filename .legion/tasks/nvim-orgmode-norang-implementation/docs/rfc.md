# RFC: nvim-orgmode Norang 对齐审计协议（Alignment Audit Protocol）

## Abstract

本文定义可实现、可测试、可评审的 A/B/C 三基线对齐审计协议，用于回答：当前实现是否与 task 记录一致，且与 Norang 原文一致。RFC 产出差异矩阵、证据链、可执行门禁与最小计划；RFC 为后续执行唯一设计真源。
...
- NG1: 本 RFC 不修改业务代码。
- NG2: 本 RFC 不改写 Norang 原文，只做语义映射。
- NG3: 本 RFC 不替代完整功能验收，仅定义审计协议与收敛门禁。

## Definitions

- `Baseline A`: 当前仓库实现与本地文档事实（代码优先）。
- `Baseline B`: `.legion/tasks/nvim-orgmode-norang-implementation/{plan.md,context.md,tasks.md}`。
- `Baseline C`: Norang 原文 `https://doc.norang.ca/org-mode.html`。
- `一致`: 语义与可观测行为无实质冲突。
...
| 主题域 | 结论 | 说明 | 关键证据 |
|---|---|---|---|
| TODO 流程与状态触发 | 部分一致 | A 已实现 `TODO->NEXT` 与 `NEXT(project)->TODO`；但未见 C 中 TODO state tag triggers 全量落地。 | A: `lua/org_punch.lua:315`, `lua/plugins/orgmode.lua:43`；C: URL `https://doc.norang.ca/org-mode.html`，摘录: "Moving a task to WAITING adds a WAITING tag" / "Moving a task to HOLD adds WAITING and HOLD tags" / "Moving a task to NEXT removes WAITING, CANCELLED, and HOLD tags" |
| Capture 模板与 clock handoff | 部分一致 | A 有 `t/r/n/m/p/j` 与 handoff；C 还含 `h/w`，且强调 `:clock-in t :clock-resume t`。 | A: `lua/plugins/orgmode.lua:62`, `lua/org_capture_norang.lua:221`；C: URL `https://doc.norang.ca/org-mode.html`，摘录: "A new habit (h)" / "(\"w\" \"org-protocol\" entry" / "Capture mode now handles automatically clocking in and out ... :clock-in t ... :clock-resume t" |
| Custom Agenda（Stuck/Projects/Standalone/Archive） | 一致 | A 已提供 block 视图并覆盖 Stuck/Projects/Standalone/Archive。 | A: `lua/plugins/orgmode.lua:95`；C: URL `https://doc.norang.ca/org-mode.html`，摘录: "Single block agenda shows the following" / "Finding stuck projects" / "Findings tasks to be archived" |
| Punch in/out 与默认任务机制 | 一致 | A 有默认任务与 punch in/out，clock out 回父任务或默认任务语义。 | A: `lua/org_punch.lua:557`, `lua/org_punch.lua:622`；C: URL `https://doc.norang.ca/org-mode.html`，摘录: "I now use the concept of punching in and punching out" / "This clocks in a predefined task by org-id" / "clocking out automatically clocks time on a parent task or moves back to the predefined default task" |
| Stuck project 判定规则 | 一致 | A 的 STUCK 判定与 C 的 project/stuck 方向一致。 | A: `lua/org_norang/rules.lua:103`；C: URL `https://doc.norang.ca/org-mode.html`，摘录: "Only projects get tasks with NEXT keywords" / "stuck projects initiate the need for marking or creating NEXT tasks" |
| time grid 与 timeline | 部分一致 | A 有 daily time grid 与 `agenda t`；C 强调 time-grid 但未定义独立 timeline 命令。 | A: `lua/plugins/orgmode.lua:36`, `lua/plugins/orgmode.lua:159`；C: URL `https://doc.norang.ca/org-mode.html`，摘录: "If I want just today's calendar view then F12 a is still faster" / "especially if I want to view a week or month's worth of information, or check my clocking data" |
| memory_only 与自动写盘语义 | 不一致 | A 存在 memory-only 与未加载文件写回并存；B 记录也冲突，语义未收敛。 | A: `lua/plugins/orgmode.lua:272`, `lua/org_norang/refresh.lua:242`；B: `context.md:145`, `context.md:149` |
| tests_ci | 不一致（验证不足） | 现有可追溯证据主要是 `luafile` 最小校验；不足以证明 R* MUST 条款均被执行验证。 | A: `.github/workflows/org-smoke.yml:1`, `tests/smoke/run.sh:1`；B: `.legion/tasks/nvim-orgmode-norang-implementation/docs/test-report.md:15` |
...
1. 进度字段冲突
   ...
   - 证据：历史问题记录见 `.legion/tasks/nvim-orgmode-norang-implementation/docs/rfc-review.md:19`；收敛后状态见 `.legion/tasks/nvim-orgmode-norang-implementation/tasks.md:26`。

2. walkthrough 的 RFC 路径错误
   - 现状：报告引用 `.legion/tasks/nvim-orgmode-norang-plugin/docs/rfc.md`，与当前 task id 不一致。
   - 证据：`.legion/tasks/nvim-orgmode-norang-implementation/docs/report-walkthrough.md:9`。

3. `tasks.md` 尾部时间戳损坏
   - 现状：审计时存在尾部孤立行时间戳损坏；本轮已修复为单一 `最后更新` 行。
   - 证据：历史问题记录见 `.legion/tasks/nvim-orgmode-norang-implementation/docs/rfc-review.md:21`；修复后见 `.legion/tasks/nvim-orgmode-norang-implementation/tasks.md:77`。

4. memory_only 历史口径冲突
   - 现状：B 同时存在“仅已加载”与“可处理未加载文件”两种叙述。
   - 证据：`.legion/tasks/nvim-orgmode-norang-implementation/context.md:145`、`.legion/tasks/nvim-orgmode-norang-implementation/context.md:149`。
...
| 条款 | 已测/未测 | 证据路径 |
|---|---|---|
| R1 | 未测 | 仅有文档声明，缺少自动化断言：`.legion/tasks/nvim-orgmode-norang-implementation/docs/rfc.md` |
| R2 | 未测 | 缺少“每条结论绑定 A 证据”的 CI 校验脚本 |
| R3 | 未测 | 缺少“不一致项必须含 B/C 证据”的自动检查 |
| R4 | 已测（静态） | 本 RFC 差异矩阵含 8 域；证据：`.legion/tasks/nvim-orgmode-norang-implementation/docs/rfc.md` |
...
- 更新：`.legion/tasks/nvim-orgmode-norang-implementation/docs/rfc.md`
- 更新：`.legion/tasks/nvim-orgmode-norang-implementation/plan.md`
- 更新：`.legion/tasks/nvim-orgmode-norang-implementation/tasks.md`
- 更新：`.legion/tasks/nvim-orgmode-norang-implementation/context.md`
...
- [x] RFC 生成完成
- [x] 对抗审查 PASS（见 `.legion/tasks/nvim-orgmode-norang-implementation/docs/rfc-review.md`）
