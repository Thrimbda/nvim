...
- `<Leader>X`：Org Capture（Norang clock handoff，替代默认 `<Leader>oc`）
...
- Agenda 视图中：`I`/`O` 已由本仓库重绑到 Norang clock in/out 实现
...
- 默认 org/agenda 的 clock in/out 映射已禁用后重绑到 Norang 实现（按键保持一致）
...
`tests/smoke/run.sh` 已纳入 Norang parity 套件（含 `norang_e2e_integrated_flow`），用于 headless、可重复的行为语义校验。
