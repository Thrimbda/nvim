...
local function org_clock_in()
  if not call_org_action("org_mappings.clock_in") then
    feedkeys("<Leader>oxi")
  end
end

local function org_clock_out()
  if not call_org_action("org_mappings.clock_out") then
    feedkeys("<Leader>oxo")
  end
end

local function org_clock_goto()
  if not call_org_action("org_mappings.clock_goto") then
    feedkeys("<Leader>oxj")
  end
end
...
function M.setup(opts)
  ...
  if not M.cfg.organization_task_id or M.cfg.organization_task_id == "" then
    vim.notify("org_punch: organization_task_id is required", vim.log.levels.WARN)
  end
end
