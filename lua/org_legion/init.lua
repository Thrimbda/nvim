...
local function command_guard(cfg, allow_dry_cleanup)
  ...
  notify(cfg, "org_legion: in error state (E_CFG_INVALID), run :OrgLegionReload", vim.log.levels.ERROR)
  return false
end

local function setup_autocmd(cfg)
  local group = vim.api.nvim_create_augroup("OrgLegion", { clear = true })
  ...
  vim.api.nvim_create_autocmd("BufWritePost", {
    ...
    callback = function(args)
      if vim.b[args.buf].org_legion_internal_write then
        vim.b[args.buf].org_legion_internal_write = false
        return
      end
      if M.state.phase == "S5" then
        return
...
local function clear_autocmds()
  vim.api.nvim_create_augroup("OrgLegion", { clear = true })
end

local function register_commands()
  pcall(vim.api.nvim_del_user_command, "OrgLegionRefresh")
  pcall(vim.api.nvim_del_user_command, "OrgLegionReload")
  pcall(vim.api.nvim_del_user_command, "OrgLegionCleanupDerivedTags")
  ...
  vim.api.nvim_create_user_command("OrgLegionReload", function()
    M.reload()
  end, { desc = "Reload org_legion config and recover from error state" })

  vim.api.nvim_create_user_command("OrgLegionCleanupDerivedTags", function(args)
...
function M.setup(opts)
  if opts ~= nil and type(opts) ~= "table" then
    ...
    notify(nil, "org_legion: invalid config (E_CFG_INVALID): setup opts must be a table", vim.log.levels.ERROR)
...
function M.reload(opts)
  if opts ~= nil and type(opts) ~= "table" then
    notify(M._cfg, "org_legion: reload failed (E_CFG_INVALID): reload opts must be a table", vim.log.levels.ERROR)
  ...
  if ok then
    notify(M._cfg, "org_legion: reload completed", vim.log.levels.INFO)
  else
    notify(M._cfg, "org_legion: reload failed (E_CFG_INVALID): " .. tostring(err), vim.log.levels.ERROR)
  end
  return ok, err
end
...
function M.cleanup_derived_tags(opts)
  ...
  if M.state.phase == "S5" and apply then
    notify(cfg, "org_legion: cleanup apply blocked in error state; use dry-run or :OrgLegionReload", vim.log.levels.ERROR)
    return { ok = false, error = { code = "E_CFG_INVALID", message = M.state.cfg_error } }
  end
  ...
  notify(
    cfg,
    string.format(
      "org_legion cleanup (%s): total=%d touched=%d changed_files=%d changed_lines=%d fail=%d skipped_conflict=%d skipped_unloaded=%d",
